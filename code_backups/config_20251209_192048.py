...
# Trading Configuration - ALL Coins (No Price Filter - BTC/ETH included)
...
MAX_COINS_TO_MONITOR = 500    # Monitor more coins (increased for all market caps)
...
# AI Model Configuration (high-precision mode)
MODEL_UPDATE_INTERVAL = 3600   # Update model every hour
BACKTEST_PERIOD_DAYS = 7       # Reduced for faster backtest
# Stricter thresholds: fewer but higher-quality signals
MIN_CONFIDENCE_SCORE = 0.60    # Required signal probability (60%+) - increased from 55%
MIN_AI_CONFIDENCE = 0.65       # Required AI confidence (65%+) - increased from 60%
# Extra filters for signal quality
SIGNAL_DOMINANCE_MARGIN = 0.15       # pump_prob vs dump_prob gap (15%+)
MIN_PRICE_MOVE_FOR_SIGNAL = 0.02     # Minimum 10m move (~2%) to allow any signal - increased from 1.5%
MIN_VOLUME_SPIKE_FOR_SIGNAL = 1.50   # Minimum volume_change multiple (e.g. 1.5x) - increased from 1.4x
...
SIGNAL_MAX_LIFETIME_HOURS = 2      # After this, open signals are marked as timeout (reduced from 4)
EARLY_TIMEOUT_ENABLED = True       # Enable early timeout if price is stuck
EARLY_TIMEOUT_MIN_HOURS = 1.0      # After this many hours with no move, allow early timeout (reduced from 1.5)
EARLY_TIMEOUT_MAX_MOVE_PCT = 0.02  # If price moved less than 2% from entry, consider it "stuck"
...
SELF_TEACHING_INTERVAL = 3600  # Run teaching session every 1 hour (3600 seconds) - FASTER
